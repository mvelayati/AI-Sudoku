...
class Solver(object):
    ...
    def solve(Sudoku, level, order_method):
        ...
        self.solution.append(deepcopy(Sudoku))

        is_solved = solve_recurse(level, order_method, timelimit)

        return [is_solved, self.num_backtracks, time.process_time() - self.time, self.solution[-1]]

    def solve_recurse(level, order_method, timelimit):
        Sudoku = deepcopy(self.solution.front())
        ...
        while(not is_solved and time.process_time() < timelimit):
            ...
            for possibility in spot.domain():
                ...
                if Sudoku.forward_prop(level, spot):
                    # save a deepcopy of puzzle each time assignment isn't illegal
                    self.solution.append(deepcopy(Sudoku))
                ...
                # made it to the end of this for loop; you are backtracking
                self.num_backtracks += 1 # continue searching possibilities for valid propogations
                Sudoku = deepcopy(self.solution[-1]) # reset the puzzle to last partial solution
